...
lotr_graph.load_edges(
    data=df, time="index", src="v", dst="w"
)
...
for windowed_graph in lotr_graph.rolling(window=250):
    ...
    fimportance.append(result.get("Frodo").__getitem__("pagerank_score"))
    gimportance.append(result.get("Gandalf").__getitem__("pagerank_score"))
